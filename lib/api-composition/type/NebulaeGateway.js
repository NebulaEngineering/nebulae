...
class NebulaeGateway {
    ...
    /**
     * Builds the shell using the downloaded API shell
     * return an observable that resolves to the builded API shell path
     * @param {string} apiPath 
     * @param {MicroApisSetupCompendium} mapiSetups 
     */
    buildApiDevelopment$(apiPath, mapiSetups) {
        //Create a src content copy to development folder
        return Rx.Observable.forkJoin(this.createSrcContentCopy$(apiPath, mapiSetups))
            .concat(this.installDependecies$(apiPath, mapiSetups));
    }
    ...
    /**
     * Creates a copy for each micro-api content into the right API shell directory
     * Returns an observable that resolves to an array of copied contents paths
     * @param {*} apiPath 
     * @param {*} apiSetups 
     */
    createSrcContentCopy$(apiPath, apiSetups) {
        return Rx.Observable.of(apiSetups)
            .mergeMap(apiSetup => Rx.Observable.from(apiSetup.microApis))
            .mergeMap(microApi => {
                const sourceDirectory = `${microApi.gitRoot}/${microApi.src}`;
                const destDirectory = `${apiPath}/${microApi.type}`;
                shell.cp('-r', sourceDirectory, destDirectory);
                return Rx.Observable.of(`${destDirectory}/${microApi.name}`);
