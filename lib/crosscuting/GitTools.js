...
class GitTools {
     ...
     */
    static clone$(repositoryUrl, outputDir, apiRepoBranch, apiRepoUser, apiRepoPsw, apiRepoRegistry) {
        const remote = (apiRepoUser && apiRepoPsw) ? `https://${USER}:${PASS}@${REPO.replace('https://', '').replace('http://', '')}` : repositoryUrl;
