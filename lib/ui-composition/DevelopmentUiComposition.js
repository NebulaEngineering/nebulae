...
class DevelopmentUiComposition {
    ...
    composeUI$() {
        ...
        return Rx.Observable.forkJoin(
            ...
            .mergeMap(([shellPath, mfeSetups, mfeContentPaths, mfePaths]) => this.buildShell$(shellPath, mfeContentPaths, mfeAssetsPaths, mfeSetups))
            ;
    }
    ...
    downloadShell$(outputDir) {
        ...
        return Rx.Observable.fromPromise(fs.remove(outputDir).then(fs.ensureDir(tmpDir)))
            .mergeMap(() => GitTools.clone$(this.shellRepo, tmpDir))
