...
class MicroserviceGenerator {
     ...
     */
    generateMicroservice$({ frontendId, apiId, projectContext, templateGitUrl, repoGitUrl }) {
        return Rx.Observable.forkJoin(
            this.DownlaodTemplate$(templateGitUrl),
            this.generateDictionary$(frontendId, apiId, projectContext, repoGitUrl)
        ).mergeMap(([repoPath, dictionary]) => Rx.Observable.concat(
            Rx.Observable.of(`Template from ${templateGitUrl} was cloned to ${repoPath}`),
            Rx.Observable.of(`Dictionary for replacement generated ${JSON.stringify(dictionary, null, 2)}`),
     ...
     * @returns {Rx.Observable}
     */
    generateDictionary$(frontendId, apiId, projectContext, repoGitUrl) {
    ...
    initializeGitRepo$(repoPath, repoGitUrl) {
        return Rx.Observable.defer(() => fs.remove(`repoPath${path.sep}.git`))
    ...
    commitAndPushGitChanges$(repoPath) {
        return Rx.Observable.create(obs => {
            require('simple-git')(repoPath)
                .add('./*')
                .commit("Template redifined")
            //.push('origin', 'master');
            obs.next(`Git repo at ${repoPath}, added all changed files and commited cahnges`);
            obs.complete();
        })
    }
    ...
    replaceDirectoriesNames$(repoPath, dictionary) {
        ...
        return Rx.Observable.bindNodeCallback(glob)(repoPath + '/**/')
            ...
            .mergeMap(directory => {
                ...
                const newDirName = directory.replace(wildcard.wildcard, wildcard.replacement);
                return Rx.Observable.bindNodeCallback(fs.rename)(directory, newDirName)
                    .mapTo(`Renamed directory ${directory} to ${newDirName}`)
            });
    }
        ...
        return Rx.Observable.bindNodeCallback(glob)(repoPath + '/**/*')
            ...
            .mergeMap(filesToReplace => {
                ...
                return Rx.Observable.bindNodeCallback(fs.rename)(filesToReplace.file, newFileName)
                    .mapTo(`Renamed file ${filesToReplace.file} to ${newFileName}`)
            });
    }
        ...
        return Rx.Observable.bindNodeCallback(glob)(repoPath + '/**/*')
            ...
            .mergeMap(file => {
                ...
                dictionary.forEach(wildcard => {
                    options.from.push(`/${wildcard.wildcard}/g`);
                    options.to.push(wildcard.replacement);
                });
                return Rx.Observable.defer(() => (replace(options)))
                    .mapTo(`Modified files contents from ${file}`)
            });
